...
contract SplitInsurance {
    //this contract is valid for a particular token itself
    ...
    */
    ...
    constructor () {
        ...
        B = address(new Tranche("Tranche B", "B"));//ERC20 tokens
    ...
    function splitRisk(uint256 amount_c) public {
        ...
        if (amount_c % 2 != 0) {// SPDX-License-Identifier: MIT
...
contract SplitInsurance {
    ...
    */
    ...
    function invest() public {
        ...
        totalTranches = ITranche(A).totalSupply() * 2;//is only minted for the insurance purpose..., also owner of the contract is this contract
    ...
    function divest() public {
        ...
        if (withdrawn_x > halfOfTranches) {
            interest += withdrawn_x - halfOfTranches;//profit from protocol x
        }
        ...
        if (withdrawn_y > halfOfTranches) {
            interest += withdrawn_y - halfOfTranches;//profit in y as well
        }
        ...
        if (!inLiquidMode) {
            if(!isInvested && block.timestamp >= T1) {
                ...
                if (block.timestamp < T1) {
                    ...
                    revert("split: call divest() first");//to avoid divest problem just use chainlink keepers
                } else {
